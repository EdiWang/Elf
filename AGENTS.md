...
- `Elf.Admin` is the single ASP.NET Core Web host for management, tags, reports, and public redirects. `Elf.Api` is a class library containing the forwarder controller, handlers, and support services. Shared URL verification helpers live in `Elf.Shared`; token generation lives in `Elf.TokenGenerator`.
- Main user flows:
...
- `Elf.Admin` is the only Web application and runtime host. It serves the Razor Pages admin UI, Admin API controllers, and public forwarder routes. It uses EF Core with lazy-loading proxies, LiteBus command/query handlers, Fluent Web Components, Alpine.js modules, and Bootstrap Icons.
- `Elf.Api` is a class library referenced by `Elf.Admin`; it contains the public `ForwardController`, its LiteBus handlers, forwarder services, cache helpers, and database startup initialization. It is not an independently published or deployed application.
...
- Keep the public redirect path efficient. Forwarder and Admin features share the `Elf.Admin` host, `ElfDbContext`, and distributed cache; preserve the existing EF Core query behavior and cache invalidation.
- Use LiteBus records for business operations:
	...
	- Register new handlers by keeping them in the Admin or Forwarder assembly; `Elf.Admin/Program.cs` registers command and query modules from both.
...
- `Elf.Data.LinkEntity` is the shared EF Core entity used by the forwarder and Admin.
...
- `ConnectionStrings:ElfDatabase` is required for the `Elf.Admin` host.
- `ConnectionStrings:RedisConnection` is optional for the combined `Elf.Admin` host; if missing, it falls back to distributed memory cache.
