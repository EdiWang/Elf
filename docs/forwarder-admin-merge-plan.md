...
> 状态：批次 0 基线记录已完成；批次 1–5 未开始。最后更新：2026-09-24。
>
...
- 用户要求先规划、分批执行、每批测试/提交/验收；研究或执行中遇到需要产品或部署取舍的问题，要先向用户说明建议并询问，不得代替用户决定。本轮用户已授权执行批次 0；后续代码合并和部署须按用户后续指示推进，生产入口切换仍须用户明确决定。
