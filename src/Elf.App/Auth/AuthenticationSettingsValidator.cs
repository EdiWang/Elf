...
public class AuthenticationSettingsValidator : IValidateOptions<AuthenticationSettings>
{
    ...
    {
        if (options.Provider is AuthenticationProvider.Local or AuthenticationProvider.External)
        {
        ...
        if (options.Provider != AuthenticationProvider.OpenIdConnect)
        {
            return ValidateOptionsResult.Fail(
                $"Authentication provider '{options.Provider}' is not supported. Supported values: Local, OpenIdConnect, External.");
        }
        ...
        {
            failures.Add(
                "Authentication:OpenIdConnect:Authority must be an absolute HTTPS URL without a query or fragment.");
        }
        ...
        {
            failures.Add(
                "Authentication:OpenIdConnect:CallbackPath must be an application-relative path that starts with a single '/'.");
        }

        if (!IsValidCallbackPath(oidc.SignedOutCallbackPath))
        {
            failures.Add(
                "Authentication:OpenIdConnect:SignedOutCallbackPath must be an application-relative path that starts with a single '/'.");
        }
        ...
        {
            failures.Add(
                "Authentication:OpenIdConnect:Scopes must contain 'openid' and cannot contain blank values.");
        }

        if (oidc.AllowedSubjects is null || oidc.AllowedSubjects.Any(string.IsNullOrWhiteSpace))
        {
            failures.Add(
                "Authentication:OpenIdConnect:AllowedSubjects cannot contain blank OIDC subject identifiers.");
        }
    ...
    private static void AddRequiredFailure(string value, string settingName, List<string> failures)
    {
        ...
        {
            failures.Add(
                $"Authentication:OpenIdConnect:{settingName} is required when Authentication:Provider is OpenIdConnect.");
        }
    }
}
